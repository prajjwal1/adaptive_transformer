...
from pretrain.lxmert_data import (InputExample, LXMERTDataset, LXMERTEvaluator,
                                  LXMERTTorchDataset)
...
def get_tuple(
    ...
    return DataTuple(
        dataset=dset, torchdset=tset, loader=data_loader, evaluator=evaluator
    )
...
valid_tuple = get_tuple(
    args.valid, valid_batch_size, shuffle=False, drop_last=False, topk=5000
)
...
def convert_example_to_features(
    example: InputExample, max_seq_length, tokenizer
) -> InputFeatures:
    """
...
class LXMERT:
    ...
    def forward(self, examples):
        train_features = [
            convert_example_to_features(example, self.max_seq_length, self.tokenizer)
        ...
        lm_labels = torch.tensor(
            [f.lm_label_ids for f in train_features], dtype=torch.long
        ).cuda()
        ...
        for key in ("obj", "attr", "feat"):
            ...
            assert visn_labels.size(0) == visn_mask.size(0) and visn_labels.size(
                1
            ) == visn_mask.size(1)
    ...
    def save(self, name):
        torch.save(
            self.model.state_dict(), os.path.join(args.output, "%s_LXRT.pth" % name)
        )
