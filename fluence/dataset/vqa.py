...
class VQADataset:
    ...
    def plot_img(self,idx):
        im = cv.imread(self.path+'val'+'2014'+'/'+ self.data[idx]['img_id']+'.jpg') # Returns array
        print("Plotting from ",self.path+self.splits[0]+'2014'+'/'+ self.data[idx]['img_id']+'.jpg')
        im = plt.imshow(im)
        im = plt.show()
        return im,self.data[idx]
        
class VQATorchDataset(Dataset):
    def __init__(self, dataset: VQADataset,MSCOCO_IMGFEAT_ROOT,tiny=False):
        super().__init__()
